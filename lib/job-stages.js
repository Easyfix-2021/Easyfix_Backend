...
const STAGES = Object.freeze({
   ...
   * `estimate-pending` still OWNS 15 for stageOfStatus/transitionAllowed
   * purposes (it is declared AFTER `pending-material` below, so it wins the
   * STATUS_TO_STAGE reverse map's last-write-wins build) — only VISIBILITY
   * (stageVisibleStatuses, the list-scoping union) is widened. A PM holding
   * only `pending-material` can therefore LIST a 15 job but the ESTIMATE
   * approve/reject action itself is gated elsewhere (isEstimateApprovable /
   * the client-only estimate routes), not by this stage's `targets`.
