...
const STAGES = Object.freeze({
  ...
  'pending-close':      { visible: [2, 20], targets: [10, 21, 6, 16] },
  /*
   * 2026-09-10 — realigned to the job_status codes the rest of the platform
   * uses. audit-complete previously claimed [3,5] and pending-feedback [10],
   * which had 3 and 10 the wrong way round versus the legacy CRM's AppConstant
   * (JOB_STATUS_AUDIT_AND_COMPLETE = 10, case 3 -> PendingFeedback,
   * JOB_STATUS_COMPLETED = 5) AND versus this platform's own CRM dashboard,
   * which already counted Audit as b['10'].
   *
   * The SLUG stays 'audit-complete' even though the label is now "Under
   * Audit": it is a URL operators bookmark and the target of the legacy
   * redirect map (PendingForCheckout = status 10 lands here, which this change
   * finally makes correct). Renaming it would break those links for a purely
   * cosmetic gain.
   *
   * Lifecycle now reads: 2/20 close -> 10 audit -> 3 feedback -> 5 completed.
   *
   * 2026-09-24 (V3 Phase 4, D7) — 1 added: the desk schedules VISIT 2 of a
   * revisit job straight from 10 back to SCHEDULED, same technician
   * (POST /admin/jobs/:id/schedule-visit-two). Source-anchored like every
   * target, so it widens ONLY a holder of this stage, and only for jobs sitting
   * at 10. Because assign/offer guard with the fixed target 1
   * (middleware/require-stage.js), it also admits those two routes on a 10
   * job for this holder. Mirror: Easyfix_CRM_UI src/lib/job-stages.ts.
   */
  'audit-complete':     { visible: [10],    targets: [3, 5, 6, 1] },
  'pending-feedback':   { visible: [3],     targets: [5, 6] },
  'completed':          { visible: [5],     targets: [] },
  'onhold':             { visible: [21],    targets: [1, 6] },
  /*
   * 2026-09-18 — status 16 "Pending for Material". It needs a stage of its own
   * because a stage-RESTRICTED user (mode 'list') can only ever see statuses
   * belonging to a stage they hold: with no stage, the PM who is supposed to
   * review these quotes could never see one. Targets are where a review sends
   * the job: 15 on approve, 2 when the client rejects, 6 if ops cancels. It
   * stays at 16 on a PM reject, which needs no target.
   *
   * 2026-09-21 (Material Request Flow v2) — `visible` widened to [16, 15]:
   * the CRM's Pending for Material list (My Orders / Jobs) now shows BOTH
   * Review Pending (16) and Approval Pending (15) rows on one tab, so a PM
   * restricted to only this stage must be able to SEE a 15 job too, not just
   * 16. This is a DELIBERATE, INTENTIONAL exception to the "every visible
   * status is unique across stages" contract noted at the top of this file —
   * `estimate-pending` still owns 15 for the LABEL (stageOfStatus — it is
   * declared AFTER `pending-material` below, so it wins the STATUS_TO_STAGE
   * reverse map's last-write-wins build). ACCESS is not single-owner:
   * stageVisible and transitionAllowed check every owned stage that lists the
   * status, so a PM holding only `pending-material` can list AND act on a 15
   * job, limited to this stage's own `targets`. The client's estimate
   * approve/reject stays client-only (isEstimateApprovable).
   * tests/job-stages-parity.test.js byte-compares this array against the
   * CRM's src/lib/job-stages.ts mirror — keep the two in lockstep.
   */
  'pending-material':   { visible: [16, 15], targets: [15, 2, 6] },
...
/*
 * STAGE_LABELS WAS REMOVED HERE (2026-09-10). Do not reintroduce it without a
 * caller.
 *
 * It claimed to be "BE-side parity with the FE stage picker" and was not:
 * 6 of its 10 labels disagreed with src/lib/job-stages.ts — "Unconfirmed" vs
 * "Unconfirmed Orders", "On Hold" vs "Orders in Followup", and four more. It
 * had ZERO consumers: the only two references in the whole repository were its
 * own definition and its own export line, and all 7 importers of this module
 * destructure other names.
 *
 * So it was a second, silently-wrong copy of a user-facing vocabulary that
 * nothing read — the worst shape for a shared definition, because the first
 * person to need a stage name here would have found something that looked
 * authoritative, was exported, and rendered the wrong words.
 *
 * IF A BACKEND SURFACE EVER NEEDS TO RENDER A STAGE NAME, take it from the
 * frontend table rather than re-typing it: the labels are what the operator
 * already reads on screen, and a second copy diverges the moment either side is
 * edited — which is exactly what happened to this one. Nothing here enforces
 * anything with a label; enforcement keys off stage_key throughout.
 */
...
function transitionAllowed(allowed, source, target) {
   ...
   */
  /*
   * Checked against EVERY owned stage that lists the source, not only
   * stageOfStatus(source): since material request flow v2, status 15 is
   * visible in BOTH `pending-material` (as Approval Pending) and
   * `estimate-pending`, so the one-owner reverse map would 403 a
   * pending-material holder on a job their own page lists. Same rule as the
   * CRM mirror (Easyfix_CRM_UI/src/lib/job-stages.ts), pinned by its parity test.
   */
  const src = Number(source);
  const tgt = Number(target);
  for (const key of allowedSet) {
    const def = STAGES[key];
    if (!def || !def.visible.includes(src)) continue;
    // Same-stage move (e.g. 2 ↔ 20 inside pending-close) — not a stage change.
    if (def.visible.includes(tgt) || (def.targets || []).includes(tgt)) return true;
  }
  return false;
}
...
function stageVisible(allowed, source) {
  if (!allowed || allowed.mode === 'all') return true;
  // Any owned stage listing the status — see transitionAllowed on 15.
  return stageVisibleStatuses(allowed.stages).has(Number(source));
}
