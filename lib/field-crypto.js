 ...
 *
 * UNSEALED VARIANT — the shape written when NO recovery key is configured:
 *   v2:<op_fp>:-:<dek_iv>:<dek_tag>:<dek_ct>::<iv>:<tag>:<ct>
 *                    ↑                        ↑
 *          the '-' SENTINEL          an EMPTY sealed-DEK field
 *
 * Still ten fields, still v2, and DELIBERATELY SELF-DESCRIBING: the sentinel is
 * a character no fingerprint can be (fingerprints are 8 lowercase hex), so an
 * unsealed envelope can never be read as a truncated one and a truncated one can
 * never be read as unsealed. The two halves are checked TOGETHER by the regex —
 * '-' demands an empty seal and a real fingerprint demands a non-empty one — so
 * an envelope naming a recovery key but carrying no seal is refused as
 * corruption rather than silently treated as "no recovery path".
 *
 ...
 *                                      REQUIRED, and fail-closed on read AND
 *                                      write.
 ...
 *                                      resolveRecoveryPublicKey(). OPTIONAL —
 *                                      see "THE SEAL IS OPTIONAL" below.
 *
 ...
 *
 * ── THE SEAL IS OPTIONAL. THE OPERATIONAL KEY IS NOT. ───────────────────
 * OWNER DECISION, 2026-09-01: run with ONE key. No recovery keypair is
 * configured anywhere — blank in local .env, blank in QA, blank in Production.
 * An earlier build refused every write in that state, which made the chosen
 * configuration an outage rather than a configuration.
 *
 * So exactly one requirement is relaxed: when NO recovery key resolves, a write
 * produces the UNSEALED envelope above instead of throwing. Everything else is
 * unchanged — the operational key missing, short or malformed still refuses on
 * read and on write, there is still no plaintext path, and a value that is not a
 * v2 envelope is still refused rather than returned.
 *
 * ⚠ OPTIONAL IS NOT BEST-EFFORT. The ONLY no-seal case is "none configured". A
 *   recovery key that IS configured and then fails to seal — a key that parsed
 *   but cannot encrypt, an RSA error, anything — THROWS. Swallowing that would
 *   write a row with no break-glass path in an environment whose whole point is
 *   that it has one, and nothing downstream could ever tell the two apart.
 *   There is no try/catch around the publicEncrypt below, on purpose.
 *
 * ⚠ WHAT NO SEAL COSTS: losing EASYFIX_FIELD_ENC_KEY means the values are
 *   unreadable and nothing can recover them. It is not a leak risk — rotate
 *   handles a leak, and never produces a plaintext copy — it is a LOSS risk.
 *
 * ⚠ AND IT IS NOT A ONE-WAY DOOR. An unsealed row can be given a seal LATER with
 *   only the OPERATIONAL key: sealToRecoveryKey() unwraps the DEK with the
 *   working operational key and seals it to a newly registered recovery public
 *   key. No old private key is involved because there is no old seal to open. So
 *   registering a recovery key next year retro-protects every row written this
 *   year, for as long as the operational key still works.
 *
...
/*
 * The rec_fp of an envelope that was written with NO recovery key configured.
 * A single '-' cannot collide with a fingerprint (8 lowercase hex) and cannot be
 * mistaken for base64, which is what makes the unsealed shape self-describing.
 * Exported so services/field-rekey.service.js can name it instead of hardcoding
 * a hyphen.
 */
const NO_SEAL = '-';
 ...
 *
 * TWO ALTERNATIVES, NOT ONE LOOSENED PATTERN. The sealed shape is exactly what
 * it always was. The unsealed shape pins the '-' sentinel AND the empty sealed
 * field to each other, in the same expression, so the pair can only ever appear
 * together: a real fingerprint with an empty seal is refused, and so is '-' with
 * a seal attached. Loosening the sealed field to `{0,}` instead would have made
 * every envelope with a dropped seal look valid — which is precisely the
 * "truncated read as unsealed" confusion the sentinel exists to prevent.
 */
const B64_FIELD = '[A-Za-z0-9+/]+={0,2}';
const FP_FIELD = `[0-9a-f]{${FP_CHARS}}`;
const SEALED = `${FP_FIELD}(?::${B64_FIELD}){7}`;
/* dek_iv, dek_tag, dek_ct — then the EMPTY seal — then iv, tag, ct. */
const UNSEALED = `${NO_SEAL}(?::${B64_FIELD}){3}:(?::${B64_FIELD}){3}`;
const ENCRYPTED_RE = new RegExp(`^${VERSION}:${FP_FIELD}:(?:${SEALED}|${UNSEALED})$`);
...
/*
 * ── WHERE THE OPTIONALITY IS DECIDED — THIS FUNCTION AND NOWHERE ELSE ───
 *
 * The recovery key a WRITE will seal to, or NULL when none is configured.
 * Synchronous by necessity — see the note above.
 *
 * Null is returned for exactly one state: no primed store key AND no env var.
 * A key that IS present and does not parse, is the wrong type, is too small, or
 * is a private half still THROWS out of recoveryFromEnv() / keyFromRow() — a
 * broken configured key is a fault, not a decision, and must not degrade into a
 * quiet unsealed write. encryptField is the only caller and does not catch.
 */
function activeRecoveryKey() {
  if (storedRecovery) return storedRecovery;
  return recoveryFromEnv();
}
...
async function resolveRecoveryPublicKey(runner) {
  ...
  /*
   * source:'none' rather than a throw. This is the OWNER'S CHOSEN STATE (see the
   * header), and the descriptor says so plainly so the boot log can state it as
   * a fact at info rather than cry wolf at warn on every start. A CONFIGURED but
   * unusable key never reaches here — it throws above, and that is the case that
   * still deserves a warning.
   */
  return { fingerprint: null, source: 'none' };
}
...
/* True when the envelope carries a sealed DEK. False means it was written while
 * no recovery key was configured: the operational key is the ONLY way in, and
 * sealToRecoveryKey() is how it gets a second one. */
function isSealed(env) {
  return env.recFp !== NO_SEAL;
}

/*
 * A row with no sealed DEK cannot be opened by a private key — there is nothing
 * for one to open. Refused with its OWN code, well before RSA is handed an empty
 * buffer and returns something about padding at the one moment somebody is
 * trying to get their data back.
 */
function notSealed(what) {
  return Object.assign(
    new Error(
      `this value was written with no recovery key configured, so it has no sealed data key`
      + ` and cannot be ${what} with one. The OPERATIONAL key is the only way in. If a recovery`
      + ` key has since been registered, seal it with sealToRecoveryKey() and the operational key.`,
    ),
    { status: 400, code: 'FIELD_NOT_SEALED' },
  );
}
...
function encryptField(plaintext) {
  ...
  //
  // The OPERATIONAL key throws here when it is missing or malformed. The
  // recovery key resolves to null when NONE IS CONFIGURED and throws when one is
  // configured and unusable — see activeRecoveryKey().
  ...
  const out = buildEnvelope({
    opFp: op.fp,
    // No key configured → the sentinel and an empty seal, TOGETHER. Not wrapped
    // in a try/catch: a configured key that fails to seal must throw, so that a
    // seal is never quietly dropped in an environment that has one.
    recFp: rec ? rec.fp : NO_SEAL,
    wrapped: gcmSeal(op.key, dek),
    sealedB64: rec ? crypto.publicEncrypt({ key: rec.key, ...OAEP }, dek).toString('base64') : '',
  ...
  if (env.opFp !== opFp) {
    ...
    throw Object.assign(
      new Error(
        ...
        + (isSealed(env)
          // The remedy names the key it wants. Two remedies exist here.
          ? ` — restore that key, or break-glass with scripts/field-recover.js and the recovery`
            + ` key ${env.recFp}`
          /* And here, exactly one does. Saying "break-glass with recovery key -"
           * would send someone to look for a key that was never created. */
          : ` — this value was written with NO recovery key, so there is no break-glass path:`
            + ` restoring operational key ${env.opFp} is the only way to read it`),
      ),
      {
        status: 500,
...
function unsealDek(env, privateKeyPem) {
  /* The guard sits HERE rather than in each caller: recoverField, the
   * break-glass rewrap and resealToRecoveryKey all reach the sealed DEK through
   * this one function, so one check covers every door. */
  if (!isSealed(env)) throw notSealed('opened');
  let priv;
  try {
...
function resealToRecoveryKey(envelope, newPublicKey, opts = {}) {
  ...
  const env = parseEnvelope(text);
  /*
   * RE-sealing replaces a seal. A row that has none is a different operation
   * with a different input — sealToRecoveryKey, below, which needs the
   * OPERATIONAL key and no private key at all. Checked before the private-key
   * requirement so the operator is told which function they want, rather than
   * being asked for a key that would not help.
   */
  if (!isSealed(env)) throw notSealed('re-sealed');
...
/*
 * ── SEALING A ROW THAT NEVER HAD A SEAL — THE RETRO-PROTECTION PATH ─────
 *
 * Give an UNSEALED envelope a break-glass door, using ONLY the operational key.
 * The DEK is unwrapped with the key the row already names and sealed to a newly
 * registered recovery public key. NO PRIVATE KEY IS INVOLVED, because there is
 * no old seal to open — which is exactly why "we shipped with no recovery key"
 * is not a one-way door: register one later and every row written in the
 * meantime can be brought under it, for as long as the operational key works.
 *
 * DELIBERATELY A SEPARATE FUNCTION FROM resealToRecoveryKey. They take different
 * keys, they answer different incidents (nothing to open vs. a leak to close),
 * and their failure modes must not be confusable at 2am. Each refuses the
 * other's input by name.
 *
 * `opts.operationalKey` defaults to EASYFIX_FIELD_ENC_KEY. A row wrapped under
 * some OTHER operational key fails with FIELD_KEY_NOT_IN_RING, naming both — the
 * same error a normal read would give, because it is the same problem.
 *
 * The operational wrap and the value ciphertext pass through untouched, so this
 * cannot break a read, and it is idempotent by fingerprint like every other
 * re-key: a row that already has a seal is refused, not double-sealed.
 */
function sealToRecoveryKey(envelope, newPublicKey, opts = {}) {
  const text = String(envelope ?? '');
  if (!isEncrypted(text)) throw notAnEnvelope('seal');
  const env = parseEnvelope(text);
  if (isSealed(env)) {
    throw cryptoError(
      `this value is already sealed to recovery key ${env.recFp} — sealToRecoveryKey only adds a`
      + ' seal to a row that has none. To REPLACE a seal (a leaked recovery key) use'
      + ' resealToRecoveryKey with the old private key',
    );
  }
  const target = toPublicKey(newPublicKey, 'the recovery public key');
  const op = opts.operationalKey == null
    ? loadOperationalKey()
    : (() => {
      const key = toOperationalKey(opts.operationalKey, 'the operational key');
      return { key, fp: fingerprint(key) };
    })();
  const dek = unwrapDek(env, op.key, op.fp);
...
module.exports = {
  ...
  // The exception: this one needs only the OPERATIONAL key, because it adds a
  // seal rather than replacing one. It is what makes "no recovery key today"
  // reversible tomorrow.
  sealToRecoveryKey,

  // The rec_fp of an envelope written with no recovery key. Exported so callers
  // can recognise an unsealed row without re-deriving the sentinel.
  NO_SEAL,
};
