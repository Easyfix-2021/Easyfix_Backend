...
/*
 * Employee code — the ONE definition of the `EF` + 6-digit format.
 *
 ...
 * copy of `/^EF\d{6}$/` or of `padStart(6, '0')` anywhere in the repo is a bug
 * by definition — tests/emp-code.test.js fails the build if one appears.
 *
 ...
 */
const EMP_CODE_RE = /^EF\d{6}$/;

/** Highest allocatable sequence — six digits, so EF999999. */
const MAX_EMP_SEQ = 999999;

/** MySQL named lock guarding generate-then-INSERT. See createUser(). */
const EMP_CODE_LOCK = 'easyfix_emp_code';

/**
 * 'EF258123' -> 258123. Anything that is not exactly this format -> null.
 *
...
function parseEmpCode(code) {
  ...
  // Safe: the regex has already proven characters 3..8 are all digits.
  return Number(code.slice(2));
}

/**
 * 258123 -> 'EF258123'. Throws above EF999999 (and on anything that is not a
 * non-negative integer).
 *
 * Throwing on overflow is the point: the failure it prevents is silently
 * emitting 'EF1000000' — seven digits, which parseEmpCode then rejects, so the
 ...
 * domain — 'EF000000' matches EMP_CODE_RE, so it must round-trip. Allocation
...
function formatEmpCode(seq) {
   ...
   * 'EF000000' instead of failing. The only caller that matters (nextEmpCode)
  ...
  if (n > MAX_EMP_SEQ) {
    throw new Error(`employee code space exhausted — ${n} exceeds ${MAX_EMP_SEQ} (EF${MAX_EMP_SEQ}); widen the format before allocating more`);
  }
  return 'EF' + String(n).padStart(6, '0');
}
 ...
 * EF000001. That is deliberate per the owner — ops seed the real codes manually
 * first, and this only ever runs ahead of them on a fresh/QA database.
 */
async function nextEmpCode(conn) {
  /*
   * REGEXP, not LIKE: LIKE 'EF______' would match 'EFABCDEF' and CAST that to
  ...
  const [[row]] = await conn.query(
    `SELECT MAX(CAST(SUBSTRING(user_code, 3) AS UNSIGNED)) AS max_seq
       FROM tbl_user
      WHERE user_code REGEXP '^EF[0-9]{6}$'`
  );
  return formatEmpCode((Number(row && row.max_seq) || 0) + 1);
}

module.exports = { EMP_CODE_RE, MAX_EMP_SEQ, EMP_CODE_LOCK, parseEmpCode, formatEmpCode, nextEmpCode };
