...
test('the status sets are exactly the owner\'s Manage Jobs buckets', () => {
  // The RULE, which is what must never drift: Open is every job status minus
  // the terminal ones, so a status added to the job model (16 Pending For
  // Material, 2026-09-18) joins the bucket here the same day it joins Manage
  // Jobs, with nobody remembering to edit a list.
  const { JOB_STATUS } = require('../services/quicksight/_shared');
  assert.deepEqual([...sources.OPEN_STATUSES],
    [...jobService.ALL_STATUS_VALUES].filter((x) => !JOB_STATUS.TERMINAL_EXCLUSION.includes(x)).sort((a, b) => a - b));
  // And today's concrete value, so a new status shows up in review rather than
  // silently widening the report.
  assert.deepEqual([...sources.OPEN_STATUSES], [0, 1, 2, 9, 10, 15, 16, 20, 21]);
...
test('open jobs: parameterised status IN (the open bucket) and NO date predicate of any kind', async () => {
  ...
  // One placeholder per open status — never an interpolated list.
  assert.equal(whereOf(p1.sql),
    `WHERE J.job_status IN (${sources.OPEN_STATUSES.map(() => '?').join(', ')})`);
  assert.deepEqual(p1.params, [...sources.OPEN_STATUSES, 2000]);
  assert.deepEqual(p1.params, [0, 1, 2, 9, 10, 15, 16, 20, 21, 2000]);
