...
test('overview composes dense bounded months without inventing QC money', async () => {
  const calls = [];
  const db = {
  ...
  assert.equal(calls.length, 3, 'fixed query count must not grow with month limit');
  ...
  assert.deepEqual(result.features, { qc: false });
  assert.equal(Object.hasOwn(result.wallet, 'inQc'), false);
  const offerCall = calls.find((call) => /FROM tbl_job_offer jo/.test(call.sql));
  assert.match(offerCall.sql, /COUNT\(DISTINCT jo\.job_id\)/, 're-offers must not inflate jobs given');
  const paidCall = calls.find((call) => /SUM\(p\.paid_amount\)/.test(call.sql));
  assert.match(paidCall.sql, /et\.job_id = j\.job_id/,
    'manual wallet credits must not be presented as paid-job earnings');
  assert.match(paidCall.sql, /j\.checkout_date_time >= \?/,
    'paid amount and completion metrics must use one completion-month cohort');
});

test('overview terminates its cursor on an all-empty historical page', async () => {
  const db = {
    async query(sql) {
  ...
  assert.equal(result.months.items.length, 6);
  assert.equal(result.months.nextCursor, null);
});

test('month jobs are paid-ledger backed, owner scoped and page bounded', async () => {
  const calls = [];
  const db = {
    async query(sql, params) {
      calls.push({ sql, params });
  ...
  assert.match(dataCall.sql, /paid_job\.fk_easyfixter_id = \?/);
