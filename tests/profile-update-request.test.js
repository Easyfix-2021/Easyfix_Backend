...
/*
 * Throwaway keys for this process. `node --test` runs each test FILE in its
 * own process, so these cannot reach another suite — and setting them BEFORE
 * the services are required is not load-order superstition: lib/field-crypto
 * reads the env on every call precisely so a late-populated environment cannot
 * freeze a permanent outage into the module.
 *
 * TWO keys, because field-crypto does envelope encryption: the operational
 * secret that wraps each value's data key, and the RECOVERY PUBLIC key that the
 * same data key is sealed to. Both are required for a WRITE — a row with no
 * break-glass path is the failure that design exists to prevent — so a suite
 * that sets only the first one cannot encrypt anything. The recovery keypair is
 * generated here and thrown away; the real one is generated by the owner and
 * its private half never touches a server. RSA-2048 rather than the
 * recommended 4096 purely so this suite stays fast; field-crypto validates the
 * floor, not a specific size.
 */
process.env.EASYFIX_FIELD_ENC_KEY = crypto.randomBytes(32).toString('base64');
process.env.EASYFIX_FIELD_RECOVERY_PUBLIC_KEY = Buffer.from(
